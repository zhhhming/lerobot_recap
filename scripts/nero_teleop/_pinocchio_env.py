"""Helpers to keep Pinocchio/Placo isolated from system ROS Python paths.

The user's shell currently exports ROS Humble Python 3.10 paths into
`PYTHONPATH` and ROS libraries into `LD_LIBRARY_PATH`. That contaminates the
Python 3.12 conda env and makes `placo` / `pinocchio` load the wrong `eigenpy`.

Call `isolate_python_env_for_pinocchio()` before importing `placo`,
`pinocchio`, or `eigenpy`.
"""
...
def isolate_python_env_for_pinocchio() -> dict[str, object]:
    """Sanitize env vars and sys.path for the current Python 3.12 process."""

...
class LatencyTracker:
    ...
    def max(self) -> float | None:
        """Return the maximum latency over the rolling window, or 0.0 if empty.

        Computed from the deque so old samples (e.g. the first compile-time
        warmup inference) age out after ``maxlen`` samples instead of polluting
        the maximum forever.
        """
        if not self._values:
            return 0.0

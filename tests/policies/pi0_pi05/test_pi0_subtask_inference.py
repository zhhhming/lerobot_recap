...
from lerobot.policies.pi0.modeling_pi0 import PI0Policy, PI0Pytorch, OPENPI_ATTENTION_MASK_VALUE
from lerobot.utils.constants import ACTION, OBS_LANGUAGE_ATTENTION_MASK, OBS_LANGUAGE_TOKENS, OBS_STATE


def test_pi0_build_subtask_seed_attention_mask_is_prefix_visible_and_seed_causal():
    model = SimpleNamespace()
    prefix_pad_masks = torch.tensor([[True, False, True]])

    mask = PI0Pytorch._build_subtask_seed_attention_mask(model, prefix_pad_masks, seed_len=3)
...
def _make_subtask_generation_model(max_decode_tokens=4):
    ...
    model._prepare_attention_masks_4d = PI0Pytorch._prepare_attention_masks_4d.__get__(model)
    model._embed_language_generation_tokens = PI0Pytorch._embed_language_generation_tokens.__get__(model)
    model._sample_subtask_token = PI0Pytorch._sample_subtask_token.__get__(model)
    model._build_subtask_seed_attention_mask = PI0Pytorch._build_subtask_seed_attention_mask.__get__(model)
    model._generate_subtask = PI0Pytorch._generate_subtask.__get__(model)
    return model


def test_pi0_generate_subtask_extends_cache_and_stops_after_eos():
...
def test_pi0_sample_actions_only_generates_subtask_when_both_inference_flags_are_enabled():
...
def test_pi0_policy_predict_action_chunk_decodes_generated_subtask_text(caplog):
    class _FakeModel:
        def __init__(self):
            self._last_subtask_tokens = None
            self.last_state = None

        def sample_actions(self, images, img_masks, tokens, masks, state, **kwargs):
    ...
    with caplog.at_level(logging.DEBUG):
        actions = PI0Policy.predict_action_chunk(
            policy,
            {
    ...
    assert torch.equal(policy.model.last_state, state)
